...
test.describe("Game Creation Flow", () => {
  test("create a game, add agents, and start", async ({ page }) => {
    ...
    await expect(page.getByText("The Suspects Are Gathering")).toBeVisible({
      timeout: 10000,
    });
  ...
  test("create a game with maximum agents (5 AI + 1 human)", async ({
    page,
  }) => {
    await page.goto(BASE_URL);
    ...
    for (let i = 0; i < 5; i++) {
      await addAgentBtn.click();
      await expect(page.locator(".count-badge")).toContainText(`${i + 2} / 6`, {
        timeout: 5000,
      });
    }
