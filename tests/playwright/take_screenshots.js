...
async function main() {
  ...
  const lobbyCtx = await browser.newContext({ viewport: { width: 1280, height: 900 } });
  ...
  const { game_id: gameId } = await (await fetch(`${BACKEND_URL}/holdem/games`, { method: 'POST' })).json();
  console.log('  Game:', gameId);

  const p1 = await (await fetch(`${BACKEND_URL}/holdem/games/${gameId}/join`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ player_name: 'Alice', buy_in: 1000 })
  })).json();
  const p2 = await (await fetch(`${BACKEND_URL}/holdem/games/${gameId}/join`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ player_name: 'Bob', buy_in: 1000 })
  })).json();
  const p3 = await (await fetch(`${BACKEND_URL}/holdem/games/${gameId}/join`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  page1.on('console', msg => { if (msg.type() === 'error') console.log('  CONSOLE ERROR:', msg.text()); });

  // Set identity before navigating
  await page1.goto(BASE_URL);
  await page1.evaluate(({ pid, pname }) => {
  ...
  await page1.screenshot({ path: `${SCREENSHOT_DIR}/03-holdem-waiting-room.png` });
  ...
  if (startBtn) {
    ...
    const startResp = await fetch(`${BACKEND_URL}/holdem/games/${gameId}/start`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  await sleep(5000);
  await page1.screenshot({ path: `${SCREENSHOT_DIR}/04-holdem-poker-table.png` });
  ...
  await page2.evaluate(({ pid, pname }) => {
    localStorage.setItem('playerId', pid);
    localStorage.setItem('playerName', pname);
  ...
  await page2.screenshot({ path: `${SCREENSHOT_DIR}/05-holdem-player2-view.png` });
